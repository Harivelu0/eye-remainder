# -*- coding: utf-8 -*-
# 20-20-20 Eye Reminder - PROD MODE (20 min interval, 20 sec countdown)
...
LOG = os.path.join(tempfile.gettempdir(), "eye_reminder.log")
...
def show_popup():
    ...
    try:
        ...
        log("show_popup error: " + traceback.format_exc())
...
def reminder_loop():
    ...
    log(f"Started ({mode} mode). Interval={INTERVAL_SECONDS}s Break={BREAK_SECONDS}s")
    while not stop_event.is_set():
        ...
        log(f"Popup result: {'continue' if should_continue else 'stop'}")
...
if __name__ == "__main__":
    try:
        log("=== eye_reminder starting ===")
        loop_thread = threading.Thread(target=reminder_loop, daemon=True)
        loop_thread.start()

        if HAS_TRAY:
            log("Tray mode active")
            start_tray()
        else:
            log("No tray, running headless")
            try:
                loop_thread.join()
